...
export function reportHtml(audit: Audit, anonymous: boolean): string {
  ...
  const tasks = report.tasks.map((task, index) => `<section><h2>${index + 1}. ${escapeHtml(task.title || 'Untitled task')}</h2><p><strong>Result:</strong> ${escapeHtml(task.outcome)} · <strong>Priority:</strong> ${escapeHtml(task.severity)}</p><dl><dt>Goal</dt><dd>${escapeHtml(task.goal || 'Not recorded')}</dd><dt>Expected steps</dt><dd>${escapeHtml(task.expected || 'Not recorded')}</dd><dt>Announcements heard</dt><dd>${escapeHtml(task.announced || 'Not recorded')}</dd><dt>Focus movement</dt><dd>${escapeHtml(task.focus || 'Not recorded')}</dd><dt>Blocker</dt><dd>${escapeHtml(task.blocker || 'None recorded')}</dd></dl></section>`).join('');
